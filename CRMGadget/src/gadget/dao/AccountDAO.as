	...
	public class AccountDAO extends BaseDAO {
		...
		public function AccountDAO(sqlConnection:SQLConnection, work:Function) {
			super(work, sqlConnection, {
				...
				search_columns: [ 'AccountName','PrimaryBillToCity','PrimaryBillToPostalCode','IndexedShortText1' ],
				display_name : "accounts",
				index: [ "AccountId", "AccountType", "OwnerId" ],
				columns: { 'TEXT' : textColumns }
			});
			stmtSelectParent = new SQLStatement();
			stmtSelectParent.sqlConnection = sqlConnection;
			stmtSelectParent.text = "SELECT * FROM Account WHERE ParentAccountId IS NULL";
			
			stmtSelectChild = new SQLStatement();
			stmtSelectChild.sqlConnection = sqlConnection;
			stmtSelectChild.text = "SELECT * FROM Account WHERE ParentAccountId=:ParentAccountId";
			
			stmtSelectAccountById = new SQLStatement();
			stmtSelectAccountById.sqlConnection = sqlConnection;
			stmtSelectAccountById.text = "SELECT * FROM Account WHERE AccountId=:AccountId";
		...
		protected override function getIndexColumns():Array{
			var indexes:Array =super.getIndexColumns();
			indexes.push("ParentAccountId");
		...
		public function getAccountParent():ArrayCollection{
			exec(stmtSelectParent);
			return new ArrayCollection(stmtSelectParent.getResult().data);
		}
		
		public function getAccountChild(parentId:String):ArrayCollection{
			stmtSelectChild.parameters[":ParentAccountId"] = parentId;
			exec(stmtSelectChild);
			return new ArrayCollection(stmtSelectChild.getResult().data);
		}
		
		public function getAccountById(accountId:String):Object {
			stmtSelectAccountById.parameters[":AccountId"] = accountId;
			exec(stmtSelectAccountById);
			var result:SQLResult = stmtSelectAccountById.getResult();
...
*/		
		...
		private var textColumns:Array = [
			...
			'PriceListId',
			'PrimaryBillToAddressExternalSystemId',
			'PrimaryBillToAddressId',
			'PrimaryBillToAddressIntegrationId',
			'PrimaryBillToCity',
			...
			'PrimaryGroup',
			'PrimaryIndustryId',
			'PrimaryShipToAddressExternalSystemId',
			'PrimaryShipToAddressId',
			'PrimaryShipToAddressIntegrationId',
			'PrimaryShipToCity',
